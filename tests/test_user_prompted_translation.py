...
class TestUserPromptedFileWatcher:
    ...
    def test_concurrent_content_addition_during_trigger(self):
        """
        Test that content added during callback is not lost.

        Regression test for race condition where content added between
        releasing and re-acquiring lock gets lost.
        """
        with tempfile.NamedTemporaryFile(mode="w", delete=False, suffix=".txt") as f:
            test_file = f.name
            # Write enough for first threshold
            f.write("initial content " * 10000)

        try:
            callback_content = []
            content_added_during_callback = threading.Event()

            def slow_callback(content: str, offset: int) -> None:
                """Callback that simulates slow translation."""
                callback_content.append((content, offset))
                # Signal that we're in callback
                content_added_during_callback.set()
                # Simulate slow translation
                time.sleep(0.5)

            token_counter = TokenCounter()
            state_file = ".test_concurrent_content.json"
            Path(state_file).unlink(missing_ok=True)
            ...
            watcher = FileWatcher(
                ...
                translation_callback=slow_callback,
                token_counter=token_counter,
                state_file=state_file,
                auto_translate=False,
            )
            ...
            # Start manual trigger in background
            trigger_thread = threading.Thread(
                target=watcher.trigger_translation_manual, daemon=True
            )
            trigger_thread.start()

            # Wait for callback to start
            content_added_during_callback.wait(timeout=2.0)

            # Add new content while callback is running
            with Path(test_file).open("a", encoding="utf-8") as file_handle:
                file_handle.write("concurrent addition " * 5000)

            # Wait for trigger to complete
            trigger_thread.join(timeout=2.0)

            # Give watcher time to process new content
            time.sleep(1.5)

            watcher.stop()

            # Verify first translation happened
            assert len(callback_content) == 1
            first_content, first_offset = callback_content[0]
            assert "initial content" in first_content
            assert first_offset == 0

            # Verify offset was advanced correctly (only by first content)
            # and new content is accumulated
            final_offset = watcher.get_last_offset()
            first_content_bytes = len(first_content.encode("utf-8"))

            # Offset should be exactly the bytes we translated
            # not the full file size
            assert (
                final_offset == first_content_bytes
            ), f"Expected offset {first_content_bytes}, got {final_offset}"

            # Verify accumulated content contains concurrent addition
            with watcher._offset_lock:
                remaining_content = watcher._accumulated_content
            assert "concurrent addition" in remaining_content
