...
class BatchTranslator:
    ...
    def translate(self):
        ...
        try:
            ...
            with open(batch_input_file, "rb") as batch_file_obj:
            ...
            while True:
                ...
                if batch.status == 'completed':
                    ...
                    logging.error(
                        f"Batch job failed with status: {batch.status}")
                    return
            ...
            if result_file_id:
                result_content = self.client.files.content(
                    result_file_id).read()

                with open(self.output_file, 'w', encoding='utf-8') as f:
                    results = result_content.decode(
                        'utf-8').strip().split('\n')
                    for res in results:
                        if not res:
                            continue
                ...
                logging.error(
                    "Batch job completed but no output file was generated.")
        finally:
